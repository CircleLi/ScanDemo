//
//  NavigationHalfScreenScanViewController.m
//  ScanDemo
//
...
@interface NavigationHalfScreenScanViewController ()
...
@implementation NavigationHalfScreenScanViewController
...
- (ScanViewController *)scanVC{
    if (!_scanVC) {
        ...
        _scanVC = tmpVC;
        _scanVC.recognizeType = RecognizeAlgorithm_BlackWhiteCode;
...
- (void)viewWillLayoutSubviews{
    ...
    self.scanVC.view.frame = CGRectMake(0, 64.0f, SCREEN_WIDTH, (SCREEN_HEIGHT - 64.0f)/2);
...
- (void)scanVC_SuccessRecogize:(ScanViewController *)scanVC barcodeInfo:(NSDictionary *)barcodeInfo{
    NSLog(@"%@______%@",barcodeInfo[LY_BlackWhiteBarcode_Key],barcodeInfo[LY_BlackWhiteBarcodeType_Key]);
}
